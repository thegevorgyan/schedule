...
function aspire_loadHtml(elm, key = "default"){	
	...
	xhttp.open('GET', 'http://local.aspire/reg/public/cache/jBQa446lNO1EqGyq.html', true);
		...
		if (this.readyState == 4) {
			if(this.status == 200){
				...
				if(json.status == "200"){					
					...
					var now = new Date();
					now.setTime(now.getTime() + 1 * 180 * 1000);
...
function ready(){
	if(getCookie('avc_ageverified') != 'true' && !botCheck()){
		var fileref=document.createElement("link");
			...
			fileref.setAttribute("href", 'http://local.aspire/reg/public/cache/jBQa446lNO1EqGyq.css');
