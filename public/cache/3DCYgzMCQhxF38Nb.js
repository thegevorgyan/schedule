...
function aspire_loadHtml(elm, key = "default"){	
	...
	xhttp.open('GET', 'http://local.aspire/reg/public/cache/3DCYgzMCQhxF38Nb.html', true);
		...
		if (this.readyState == 4) {
			if(this.status == 200){
				...
				if(json.status == "200"){
					...
					window.location.href = "http://aspirehookah.com";
				} else {
					if(val == true){
...
function ready(){
	if(getCookie('avc_ageverified') != 'true' && !botCheck()){
		var fileref=document.createElement("link");
			...
			fileref.setAttribute("href", 'http://local.aspire/reg/public/cache/3DCYgzMCQhxF38Nb.css');
