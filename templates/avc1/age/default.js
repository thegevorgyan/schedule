...
function aspire_processData(val){
	...
	xhttp.open('GET', '{ASPIREURL}check?' + collectedDataString, true);
	xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
	xhttp.onreadystatechange = function() {
				...
				if(json.status == "200"){
					...
					setTimeout(function(){
						...
						setTimeout(function(){
							...
							xhr.open('GET', '{ASPIREURL}storage?' + storedDataString, true);
					...
					"{REJURL}" != "" ? window.location.href = "{REJURL}" : document.write(json.status_message);
				}
			} else {
				setCookie("aspire_token", aspire_token, 365);
			}
...
function showPopup(){
	var fileref=document.createElement("link");
			...
			fileref.setAttribute("href", '{ASPIREURL}cache/{KEY}.css');
...
function ready(){
	...
	if(!created_new_token){
		...
		var xhttp = new XMLHttpRequest();
		xhttp.open('GET', '{ASPIREURL}compare?' + collectedDataString, true);
		xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
		xhttp.onreadystatechange = function() {		
			if (this.readyState == 4 && this.status == 200) {
